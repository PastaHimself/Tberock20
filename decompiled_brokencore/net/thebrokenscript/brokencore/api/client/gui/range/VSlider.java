...
extends AbstractSlider<T> {
    ...
    public /* synthetic */ VSlider(ClosedRange closedRange, Comparable comparable, SpriteScalingSettings spriteScalingSettings, ResourceLocation resourceLocation, SpriteScalingSettings spriteScalingSettings2, ResourceLocation resourceLocation2, ResourceLocation resourceLocation3, ResourceLocation resourceLocation4, Function2 function2, int n, DefaultConstructorMarker defaultConstructorMarker) {
        if ((n & 0x100) != 0) {
            function2 = VSlider::_init_$lambda$0;
        }
    ...
    private final float calcDeltaScalar() {
        return 1.0f / (float)(this.getH() - this.getGrabberLength() - this.getBgScaling().getH());
    }

    public VSlider(@NotNull ClosedRange<T> range, @NotNull T step, @NotNull SliderSettings sliderSettings, @NotNull Function2<? super T, ? super Float, Unit> changeCallback) {
        Intrinsics.checkNotNullParameter(range, (String)"range");
        Intrinsics.checkNotNullParameter(step, (String)"step");
    ...
    public void setGrabberLength(int value) {
        ...
        this.getDragBox().setH(Math.min(value, this.getGrabber().getH()));
    ...
    public void setH(int value) {
        if (value != this.getH()) {
            super.setH(Math.max(this.getGrabberLength(), value));
            this.deltaScalar = this.calcDeltaScalar();
        } else {
            super.setH(Math.max(this.getGrabberLength(), value));
        }
    ...
    public Vector2i grabberMoved(int localMouseX, int localMouseY, int localClickX, int localClickY) {
        int ofs = Math.clamp((long)(localMouseY - localClickY - this.getGlobalY()), 0, Math.max(this.getH() - this.getGrabber().getH(), 1));
        return new Vector2i(0, ofs);
    }

    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
        if (this.isMouseOver((int)mouseX, (int)mouseY)) {
            int ofs = Math.clamp((long)this.getGrabber().getY() - (long)scrollY * (long)4, 0, Math.max(this.getH() - this.getGrabber().getH(), 1));
            this.getGrabber().setY(ofs);
            this.getDragBox().setY(ofs);
    ...
    public void resetGrabber() {
        if (this.getGrabber().getY() != 0) {
            this.getGrabber().setY(0);
            this.getDragBox().setY(0);
    ...
    public float getLerpDelta() {
        return (float)this.getGrabber().getY() * this.deltaScalar;
    }
