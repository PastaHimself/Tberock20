...
export function init() {
  ...
  register("be_command", {
    onPlayerInteract(ev) {
      ...
      try { ev.player.onScreenDisplay.setTitle(`§7${line}`, { stayDuration: 20 }); } catch {}
  ...
  register("be_portal_extender", {
    onPlayerInteract(ev) {
      try { ev.player.onScreenDisplay.setTitle("§5EXTENDER LINKED", { stayDuration: 20 }); } catch {}
    }
  });
  ...
  register("be_exit", {
    onPlayerInteract(ev) {
      try { ev.player.onScreenDisplay.setTitle("§aEXIT?", { stayDuration: 20 }); } catch {}
    }
  });

  register("be_a_flower", {
    onPlayerInteract(ev) {
      try { ev.player.playSound("thebrokenscript:tether_bloom_idle"); } catch {}
      try { ev.player.onScreenDisplay.setTitle("§d...", { stayDuration: 15 }); } catch {}
    }
  });
...
function makeJimTrigger(stage) {
  return {
    onPlace(ev) {
      // Jimmy arena trigger stages — full choreography in Chunk 12; stage marker only
