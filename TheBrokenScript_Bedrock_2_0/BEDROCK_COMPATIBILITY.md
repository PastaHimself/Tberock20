...
## Source-backed Null and chunk follow-up (2026-09-16)

The implementation was checked against the extracted source classes `NullMazeEntity.java`, `NullFlyingEntity.java`, `MazeHitGoal.java`, `PlayerUtil__PlayerCheckDSLKt.java`, `PlayerExt.java`, `ClientPlayerDSLKt.java`, `ChunkRemoverConditions.java`, `ChunkRemoverEntity.java`, `ChunkHelper.java`, `ChunkUtil.java`, and `ChunkCommands.java`. The persistent placement/break ledger also follows `ChunkTrackerHandler.java` and `PlayerModifiedChunksData.java`; force-load callsites in `StructureUtil.java`, `CorruptedCommandBlockHandler.java`, and `LevelLoadHandler.java` are inventoried in A-034.

The supported API surfaces used by the follow-up are [Microsoft's Entity Script API](https://learn.microsoft.com/en-us/minecraft/creator/scriptapi/minecraft/server/entity?view=minecraft-bedrock-stable) (`getViewDirection`, `getHeadLocation`, `isSneaking`, `applyDamage`, `teleport`, and validity checks), [Dimension](https://learn.microsoft.com/en-us/minecraft/creator/scriptapi/minecraft/server/dimension?view=minecraft-bedrock-stable) (`getBlockFromRay`, `getTopmostBlock`, `getLightLevel`, `getSkyLightLevel`, `isChunkLoaded`, `fillBlocks`, `runCommand`, and `getEntities`), [BlockVolume](https://learn.microsoft.com/en-us/minecraft/creator/scriptapi/minecraft/server/blockvolume?view=minecraft-bedrock-stable), [BlockPermutation](https://learn.microsoft.com/en-us/minecraft/creator/scriptapi/minecraft/server/blockpermutation?view=minecraft-bedrock-stable), [world after-events](https://learn.microsoft.com/en-us/minecraft/creator/scriptapi/minecraft/server/worldafterevents?view=minecraft-bedrock-stable), and [System](https://learn.microsoft.com/en-us/minecraft/creator/scriptapi/minecraft/server/system?view=minecraft-bedrock-stable). These are supported in the repository's pinned 1.26.50 Preview declaration line; the exact in-game behavior still requires a matching engine smoke run.

Bedrock's `Dimension.getLightLevel` reports total brightness rather than Java's `LightLayer.BLOCK`, so the chunk spawn gate deliberately fails closed when the value is unavailable and rejects any total-brightness value ≥ 1. `Dimension.isChunkLoaded` is used as a safety guard, not as a forced-load API. `fillBlocks` clears loaded blocks, and the stable `runCommand("clone ... replace move")` route shifts 16-block sections top-down; Bedrock remains responsible for block entities, scheduled ticks, light invalidation, chunk tickets, and client chunk packets. No unsupported raw Java chunk manipulation is attempted.
