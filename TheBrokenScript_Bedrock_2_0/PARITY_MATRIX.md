...
| Bosses (Integrity P1–3, Jimmy, Kerfur) | boss/, entity/integrity/phase1..3, xcsf arena | Integrity Arena/Phase source model + Stage2 generator/Stage2Util audits; source-backed Phase 3 ring/boundary controller, GroundAttack/GroundArm adapter, complete multi-attack runtime, and damage/death lifecycle; source-specific Tether/VoidTentacle controller; fabricated P1/P2 health-threshold transitions removed | in_progress | approximation | 32 focused JS regressions + JSON/static scanner; runtime CI/device smoke pending | Phase 3 now includes the inclusive 251-candidate ring, 100–123 ranges, three presets, boundary countdown/native void terminal damage, GroundAttack/GroundArm timing, Fireball/TentacleSwipe/Gravity/Tentacles selection and dispatch, player/fireball damage caps, hurt-frame gate, mace parry, and 298-tick delayed cleanup. Automatic arena start/participant transfer, exact void_mass attribution, cleanup/cutscene transport, Stage2 generator/path placement, exact owner-id synchronization/bounding-box contact, Jimmy, and Kerfur parity remain pending. |
| Projectiles | chord_projectile, integ_fireball, integrity_ball dmg type | Chord source-specific runtime correction (Chunk 26) + Phase 3 fireball runtime; explicit BrokenCore damage adapter | ported (slices) | approximation | 32 focused JS regressions; JSON/static scanner PASS; runtime CI/device smoke pending | Chord uses source speed/distance/impact branches and gravity restoration; `UwuableArrow#setBaseDamageFromMob` is absent, so Bedrock 6 damage remains an explicit adapter. Bone-muzzle, bounding-box collision, and renderer offset application are documented engine differences. |
