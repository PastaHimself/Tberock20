...
| Bosses (Integrity P1–3, Jimmy, Kerfur) | boss/, entity/integrity/phase1..3, xcsf arena | Integrity Arena/Phase source model + Stage2 generator/Stage2Util audits; source-backed Phase 3 ring/boundary controller; source-specific Tether/VoidTentacle controller; fabricated P1/P2 health-threshold transitions removed | in_progress | approximation | JS regression tests + Stage2 NBT/Stage2Util audits + add-on CI | Phase 3 now models the inclusive 251-candidate ring, 100–123 ranges, three presets, same-dimension boundary countdown, and terminal native void damage. The exact 428-tick FinalCutscene schedule is modeled, but Java custom overlay/camera/packet behavior is unavailable. Chunk 22 exposes the source SCALE as persisted client-synced `thebrokenscript:scale` with event-backed `minecraft:scale` groups (1..5); fixed presets use scale 2. Automatic arena start/participant transfer, exact void_mass attribution, cleanup/end choreography, Stage2 generator/path placement, ground-arm owner propagation, Jimmy, and Kerfur parity remain pending. |
