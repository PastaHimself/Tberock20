...
| Bosses (Integrity P1–3, Jimmy, Kerfur) | boss/, entity/integrity/phase1..3, xcsf arena | Integrity Arena/Phase source model + source-backed Stage2 generator/Stage2Util audits; source-specific Tether/VoidTentacle controller; fabricated P1/P2 health-threshold transitions removed | in_progress | approximation | JS regression tests + Stage2 NBT/Stage2Util audits + add-on CI | Source-backed model covers phase order, 150-block participant capture, Arena liveness, P1 1080/100/20/10/10 semantics, Stage2 floor bands/mappings, Stage2Util cell-center/chunk selection/safe-floor scan/tether exclusion semantics, generator layer/template selection, P3 center/boundary/tentacle/end predicates, and Tether/VoidTentacle goal/sweep contracts. The controller now ticks Stage2/void_shadow, preserves Tether heartbeat/no custom melee, and applies VoidTentacle target filters, 1–5 scale roll fallback, 25-tick cooldown, melee/sweep damage, sticking, and source damage immunities. Automatic arena start, Stage2 generator/path placement, exposed Bedrock SCALE/visual scale, ground-arm owner propagation, P3 runtime/cutscene, Jimmy, and Kerfur parity remain pending. |
