...
| Bosses (Integrity P1–3, Jimmy, Kerfur) | boss/, entity/integrity/phase1..3, fractured/, Kerfur sources | Integrity Arena/Phase source model + Phase 3 ring/boundary/GroundAttack/GroundArm/multi-attack/damage-death runtime; dedicated Fractured/Rock attack runtime (Chunk 28); fabricated P1/P2 health-threshold transitions removed | in_progress | approximation | 89 JS regressions + 67 validator tests on preceding repair run; GitHub Actions run 33323628395 code checks PASS; artifact uploads quota-blocked; runtime smoke pending | Phase 3 preserves ring, boundary, attacks, caps, hurt gate, mace parry, and 298-tick cleanup. Jimmy now preserves 100-tick delay, selector weights, attack lengths, stomp/slam/rock/air-lift constants, Rock owner exclusion and dedicated ownership. Fractured multipart parts/sub/leg, FracturedRoam, Kerfur, Java transport, and exact bone contact remain pending. |
| Projectiles | chord_projectile, integ_fireball, fractured/Rock, integrity_ball damage type | Chord source-specific runtime correction (Chunk 26) + Phase 3 fireball runtime + dedicated Fractured/Rock runtime (Chunk 28); explicit BrokenCore arrow-damage adapter | ported (slices) | approximation | 11 Jimmy regressions; preceding CI typecheck/full regressions/validators passed; GitHub Actions run 33323628395 code checks PASS; artifact uploads quota-blocked | Chord uses source speed/distance/impact branches and gravity restoration; Rock preserves source 15 damage, speed 8, impact AOE, owner exclusion, and AirLift pulse. Bone-muzzle, multipart/bounding-box differences, and missing BrokenCore formula remain documented adapters. |
