...
| Bosses (Integrity P1–3, Jimmy, Kerfur) | boss/, entity/integrity/phase1..3, xcsf arena | Integrity Arena/Phase source model + existing boss controller; fabricated P1/P2 health-threshold transitions removed | in_progress | unknown | JS regression tests + add-on CI | Source-backed model now covers phase order, 150-block participant capture, Arena liveness rule, P1 1080/100/20/10/10 semantics, current Stage2 floor bands/mappings, and current P3 center/boundary/tentacle/end predicates. Automatic arena start, Stage2 generator/tether runtime, P3 runtime/cutscene, Jimmy, and Kerfur parity remain pending. |
...
| Structures (source corpus + shaft jigsaw + xcsf) | structure/, xcsf_structure/ | Six-piece Shaft now has source-backed NBT + native Jigsaw pools/structure; remaining structure corpus and XCSF reconstruction still pending; Integrity arena source audit/model started | in_progress | unknown | six Shaft source/BP Git blob SHAs match; JSON + binary-NBT connector validators present; Integrity model regression tests added | Do not treat native Shaft assembly or the Arena source model as full corpus/world-placement/XCSF runtime parity |
