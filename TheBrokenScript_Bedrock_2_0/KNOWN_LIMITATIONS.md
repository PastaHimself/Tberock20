...
15. **Jimmy attack animation and particle presentation are not exact** — Chunk 28 preserves the recovered attack/timing/damage contract with a dedicated runtime, but keyframe trigger timestamps, limb origins, and the Rock block-particle burst remain explicit Bedrock adapters (A-015).

16. **Jimmy multipart and FracturedRoam host hierarchy are adapted** — Chunk 29 preserves the six logical part dimensions/offsets, distinguishes FracturedPartEntity head/chest behavior from FracturedSubEntity leg delegation, applies the source-specific positive/negative body-yaw transforms, guards Roam promotion until its 149-tick RISING state reaches NORMAL, and preserves the 103-tick SWITCHING duration. Bedrock uses a root collision envelope plus runtime conceptual AABB filtering because it has no Java child part entities or parent/part identity. Continuous projectile sweep, exact rendered bone contact, JimArena arena/camera/music choreography, and the remaining FracturedRoam underground/dig/despawn lifecycle remain unsupported or deferred (A-016).
