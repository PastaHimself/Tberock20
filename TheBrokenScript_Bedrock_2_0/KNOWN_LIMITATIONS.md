...
15. **Jimmy attack animation and particle presentation are not exact** — Chunks 28 and 33 preserve the recovered attack/timing/damage contract with a dedicated runtime. Keyframe trigger timestamps and limb origins remain explicit adapters; the Rock block-impact burst now uses a source-counted moon-stone Bedrock emitter, with emitter lifetime/physics as the remaining presentation difference (A-015, A-017).

16. **Jimmy multipart and FracturedRoam host hierarchy are adapted** — Chunks 29–31 preserve the six logical part dimensions/offsets, distinguish FracturedPartEntity head/chest behavior from FracturedSubEntity leg delegation, apply the source-specific positive/negative body-yaw transforms, guard Roam promotion until its 149-tick RISING state reaches NORMAL, preserve the 103-tick SWITCHING duration, and adapt the underground/dig/despawn lifecycle plus JimArena schedule. Bedrock uses a root collision envelope plus runtime conceptual AABB filtering because it has no Java child part entities or parent/part identity. Continuous projectile sweep, exact rendered bone contact, bossbar/camera packets, and guaranteed looping-music control remain unsupported or deferred (A-016).
