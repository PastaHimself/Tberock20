...
2. **Bedrock implementation**: the resource pack carries the source textures. `screen_overlay.js` sends `tbs:screen/<texture-id>` through the player's title channel; `hud_screen.json` hides only titles with that prefix and `tbs_screens.json` draws the named image above the VHS layer. `tools/generate_screen_ui.py` builds fixed UV controls for the 4-frame TBE Curious, 22-frame transition, and 5-frame Obliteration 2 sheets. One unusually long source filename is deployed as `garfunkle_sealed.png` to meet Creator Tools' 100-character path limit, with an explicit UI mapping from its original screen ID. The source PNG bytes remain unaltered. Normal titles keep their vanilla rendering.
...
5. **Command block screen**: the source code-entry and confirmation menus are recreated as `ModalFormData` and `MessageFormData` on interaction with `thebrokenscript:command`. Input is compared exactly with the saved world code; success checks the initiator below the block, marks the code applied, advances inventory corruption once, and offers confirmation. Confirming starts the existing Integrity arena after 100 ticks at a source-range offset from the block. Form callbacks recheck the block and dimension; a canceled confirmation leaves `codeApplied` set, as in Java. See `tests/command_block_screen.test.mjs`. [Microsoft's ModalFormData](https://learn.microsoft.com/minecraft/creator/scriptapi/minecraft/server-ui/modalformdata?view=minecraft-bedrock-stable) and [MessageFormData](https://learn.microsoft.com/minecraft/creator/scriptapi/minecraft/server-ui/messageformdata?view=minecraft-bedrock-stable) define the API; source `CorruptedCommandBlockPacket.java` and `CorruptedCommandBlockConfirmPacket.java` define the progression.
6. **Remaining difference**: original OS dialogs and Java container menus cannot be installed through a Bedrock add-on. The code and confirmation menus use native Bedrock form chrome rather than their source background PNGs. Null menu images are brief visual replicas without inventory slots; source `polaroid` and paper interactions still use native forms. The Integrity arena adapter starts after the source's combined delay, but its sky/command feedback is not an exact native Java client effect. Other copied screen textures are available for future event triggers but are not claimed to be active. Title-based routing may compete with unrelated titles that arrive at the same tick, and actual display, timing, UI scaling, and touch behavior require a Bedrock 1.26.50 Preview smoke run.
