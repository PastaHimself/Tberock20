...
2. **Source evidence**: `source_extracted/assets/thebrokenscript/animations` (35 files/197 entries), `source_extracted/assets/thebrokenscript/particles` (9 definitions), `source_extracted/data/thebrokenscript/jukebox_song` (12 songs), `source_extracted/assets/thebrokenscript/sounds.json`, `decompiled/net/thebrokenscript/registry/TBSMenus.java`, `decompiled/net/thebrokenscript/entity/{FarawayEntity,NoTextureEntity}.java`, `decompiled/net/thebrokenscript/block/NullStructureBlock.java`, `decompiled/net/thebrokenscript/mixins/features/vfx/VoidFogMixin.java`, the current `BP/` presentation runtimes, and the current `RP/` animation/audio/UI trees.
3. **Validation**: `tools/validate_presentation.py` compares animation names/lengths/loop flags and model references, particle identifiers/resources/lifetimes/counts, inventories exact Java provider/callsite families, compares every shared sound cue, verifies all 12 songs, all 10 record items, all ten Java menu registrations, the three UI files, overlay scope, and supported camera calls. The check is run by `bedrock-addon-check`; focused tests now also lock Faraway burst cardinality/spread, per-tick Wretched emission, Null Structure Creative/either-hand gating, player-local particle visibility, and Library Paper probability/volume.
4. **Replacement design**: all seven registered Java particle provider callsite families now have Bedrock runtime bridges. `fardaway` preserves the funny-setting branch, 50-particle ±3 burst, and terminal 555-particle ±2 null/eyes burst; `wretched_particle` emits two ±3 particles every source Follow/NoTexture tick; Null Structure uses an invisible, non-colliding marker block plus a Creative/either-hand-gated player-local marker particle; Library Paper runs a 1% player-local trial in the source ±16/±8/±16 volume. Client-local Java particle paths use `Player.spawnParticle`; server/world particle paths continue to use `Dimension.spawnParticle`. The Rock block-impact adapter remains source-counted at 400 particles. Ten Bedrock record components use source durations and bare `sound_definitions.json` keys; the two Lilly themes remain script-only; arena `SoundInstance` handles remain player-owned and cleaned up.
5. **Player-visible difference**: Paper originates from Java's client `FogRenderer.setupFog` render-frame hook, while Bedrock Script scheduling is tick-based, so the 1% Paper trial runs at 20 Hz and cannot reproduce FPS-dependent source cadence. Paper's crossed-quad sinusoidal renderer remains a billboard/dynamic-motion approximation. Java Null Structure exposes a full selection shape only to Creative players holding the marker; Bedrock selection boxes are block-global, so the shipped invisible marker keeps a static selectable outline while collision is disabled. `follows_particle` and `revuxor_particle` stay resource-only because no Java provider/callsite exists in the extracted source. `video/alpha3` remains intentional source-only evidence. Java GeckoLib controller transitions, server-visible rendered-bone transforms, exact melee/contact origins, FancyAudio fading/attenuation and reload mixing, actual network disconnects, custom glyph pages, and the Java GLSL pipeline remain engine-specific or blocked.
6. **Docs checked**: [Entity.playAnimation](https://learn.microsoft.com/minecraft/creator/scriptapi/minecraft/server/entity?view=minecraft-bedrock-stable), [Dimension.spawnParticle](https://learn.microsoft.com/minecraft/creator/scriptapi/minecraft/server/dimension?view=minecraft-bedrock-stable), [Player.spawnParticle](https://learn.microsoft.com/minecraft/creator/scriptapi/minecraft/server/player?view=minecraft-bedrock-stable), [EntityEquippableComponent](https://learn.microsoft.com/minecraft/creator/scriptapi/minecraft/server/entityequippablecomponent?view=minecraft-bedrock-stable), [custom sounds](https://learn.microsoft.com/minecraft/creator/documents/addcustomsounds?view=minecraft-bedrock-stable), [`minecraft:record`](https://learn.microsoft.com/minecraft/creator/reference/content/itemreference/examples/itemcomponents/minecraft_record?view=minecraft-bedrock-stable), and the [Camera Script API](https://learn.microsoft.com/minecraft/creator/scriptapi/minecraft/server/camera?view=minecraft-bedrock-stable). The repository's exact 1.26.50 Preview typings remain the executable compatibility gate.
7. **Parity class**: `VALIDATED_HIGH_PARITY` for particle trigger/count/spread/scope contracts where Bedrock exposes an equivalent surface; `VALIDATED_APPROXIMATION` for Paper render-frame cadence/custom flutter and Null Structure's per-player selection shape; `ENGINE_UNSUPPORTED` or `DEFERRED_UNSAFE` for exact Java controller/render-bone, custom-font, shader, audio-mixer, and multiplayer rendering behavior.
