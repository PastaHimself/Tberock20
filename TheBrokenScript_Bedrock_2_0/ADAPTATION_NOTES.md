...
## A-024 — Horror event and chat engine boundary

1. **Source feature**: TBSEvents and TBSChatResponses, including event scheduling, aliases, response side effects, and lifecycle hooks.
2. **Source behavior**: Java evaluates the horror-event chance every server tick, selects a random player, applies dimension/arena/class gates, and weights event candidates by inverse use count. Chat responses clean non-alphanumeric punctuation, apply ordered aliases with response-specific full/substring/case rules, and defer most effects by 100 ticks. Normal responses broadcast to the world; Fever responses are sender-only; the player's original chat is not canceled.
3. **Bedrock adaptation**: horror_event_model.js and horror_chat_model.js hold side-effect-free source contracts. horror_events.js and horror_chat.js adapt them to the Script API with guarded lifecycle subscriptions, persistent world dynamic-property state, system.run/system.runTimeout, titles/actionbars/effects/entities, and safe native API fallbacks.
4. **Player-visible difference**: OS/window title mutation, Java client UI/shader/packet effects, and exact desktop rendering are represented by the closest supported in-game effect and are not claimed as exact parity.
5. **Validation**: 86 event IDs and 42 chat IDs are checked against decompiled registry order; model behavior, runtime registration, lifecycle tokens, JavaScript type checking, and the complete GitHub Actions matrix are required gates.
6. **Parity class**: VALIDATED_APPROXIMATION overall; source matching/registry semantics are high parity, while desktop/client-only surfaces remain explicit approximations.
