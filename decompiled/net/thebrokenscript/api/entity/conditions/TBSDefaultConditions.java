...
implements SpawnConditions {
    ...
    public <T extends Entity> SpawnPlacements.SpawnPredicate<T> predicate() {
        return (arg_0, arg_1, arg_2, arg_3, arg_4) -> TBSDefaultConditions.predicate$lambda$0(this, arg_0, arg_1, arg_2, arg_3, arg_4);
    }
    ...
    private static final boolean predicate$lambda$0(TBSDefaultConditions this$0, EntityType entityType, ServerLevelAccessor world, MobSpawnType reason, BlockPos pos, RandomSource random) {
        block14: {
            block15: {
                ...
                if (Intrinsics.areEqual((Object)entityType, (Object)TBSEntities.XXRAM_2DIE.value())) {
                    return !LevelExt.INSTANCE.getVars((LevelAccessor)world).getHasRam2DieJoined() && LevelExt.INSTANCE.getVars((LevelAccessor)world).getHasTriggeredRam2Die();
                }
                if (!Intrinsics.areEqual((Object)entityType, (Object)TBSEntities.PHANTOM_PLAYER.value())) break block14;
                if ((double)random.nextFloat() > 0.0045 + (double)LongExt.INSTANCE.eventFrequency(world.getLevel().getGameTime())) break block15;
                ServerLevel serverLevel = world.getLevel();
                Intrinsics.checkNotNullExpressionValue((Object)serverLevel, (String)"getLevel(...)");
                if (!EntityLookupExtImplKt.byClass((LevelEntityGetterAdapter)EntityFinder.getEntities((ServerLevel)serverLevel), PhantomPlayerEntity.class).isEmpty()) break block15;
                ServerLevel serverLevel2 = world.getLevel();
                Intrinsics.checkNotNullExpressionValue((Object)serverLevel2, (String)"getLevel(...)");
                if (!(!EntityLookupExtImplKt.byClass((LevelEntityGetterAdapter)EntityFinder.getEntities((ServerLevel)serverLevel2), HetzerEntity.class).isEmpty())) break block14;
            }
            return false;
        }
        return true;
