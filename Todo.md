...
- [~] Verify all spawn-director rules against the source conditions rather than relying solely on the historical Chunk 17 category audit.
  - Evidence (2026-09-20): `CircuitStalkConditions.java` exposed two source-parity defects in the shipped circuit rule: Bedrock inverted Java's required `isNullHere` gate and marked the rule successful without spawning `circuit_stalk`. The BP and authoring rule now require `isNullHere`, honor the source peaceful/`doMobSpawning`/flat-world gates, evaluate candidate cave/light/biome/exclusion conditions, summon the entity before mutating `circuitSpawnDelay`/`hasCircuitSpawned`, and lock those invariants in `tests/entity_family_fidelity.test.mjs`. The broader spawn-director family audit remains `[~]` pending the remaining source conditions and a real engine smoke run.
