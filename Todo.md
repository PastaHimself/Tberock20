...
- [~] Verify all spawn-director rules against the source conditions rather than relying solely on the historical Chunk 17 category audit.
  - Evidence (2026-09-20): a focused source audit of `CircuitStalkConditions`, `NullConditions`, `TBEConditions`/`TBEAmbushConditions`, `CurvedConditions`, `HerobrineConditions`, `ObliterationConditions`, and `AnomalyConditions` found and fixed multiple shipped divergences. Circuit had an inverted `isNullHere` gate and could report success without spawning; Null omitted peaceful/`doMobSpawning`/flat/candidate gates and `NullInvadeBase` exclusion; TBE read moon phase from a Dimension instead of the World and omitted source gates; Herobrine omitted the shrine prerequisite; Obliteration omitted daylight/sky gates; Curved did not preserve cave parity; and the anomaly adapter could naturally spawn `SUB_ANOMALY_2` even though only `SUB_ANOMALY_1` is registered with `ANOMALY`. BP/authoring copies and `tests/entity_family_fidelity.test.mjs` now lock these source-backed invariants. The broader spawn-director audit remains `[~]` for the remaining condition families and real 1.26.50 Preview engine evidence.
